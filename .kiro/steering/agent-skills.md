---
inclusion: always
description: "Global engineering rules from the agent-skills pack (Kiro-native steering). Enforces commit safety, code standards, role and skill boundaries, workflow discipline, and policy-as-code."
---

# Agent Skills - Global Engineering Rules (Kiro Steering)

## Source of Truth

All rules originate from `core/rules/code.md`. This steering file is a Kiro-native mirror; when it and the source disagree, the source wins.
...
- Kiro-native hooks live at `.kiro/hooks/` (`role-gate.json`, `policy-check.json`, `trace-span.json`) — they enforce these checks at runtime.
- A2A discovery: `core/a2a/.well-known/agent-registry.json`; full lifecycle via the `agent-a2a-protocol` skill; emit structured handoffs from `core/contracts/schemas/`.

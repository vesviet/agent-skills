...
- rollback strategy is clear — prefer **Argo Rollouts** canary/blue-green with automated metric analysis for progressive delivery; define SLO analysis templates (error rate <0.1%, p99 latency within 20% of baseline) that automatically abort a rollout on regression
- user-facing docs or release notes are updated if required
- **SLO release gate**: confirm error budget is not in burn-alert before releasing to production — do not release into an active burn-rate alert
- **SBOM**: confirm the release artifact includes a CycloneDX 1.6 SBOM (generated by CI) attached to the release tag or OCI image manifest — enables sub-5-minute blast radius analysis in future incidents

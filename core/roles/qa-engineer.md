...
- [ ] **Test Scope Right**: the Testing Trophy is applied (heavy integration, focused unit, lean E2E); mutation score ≥ 75-80% via Stryker for critical libraries.
- [ ] **No Live LLM in CI**: LLM API calls are stubbed with vcr-style fixtures; never call live providers in CI.
- [ ] **No Brittle E2E for Unit Logic**: unit-level decisions are covered by unit tests, not E2E.
- [ ] **Golden-Set Stability**: a model update does not degrade output on the golden set; regressions are release-blocking.
- [ ] **HITL Trigger Tested**: fallback-to-human paths are tested explicitly; hallucination boundary inputs have tests.
- [ ] **Skipped Tests Disclosed**: skipped tests in the test report are documented with rationale; reports with skipped > 0% without a rationale are rejected.
- [ ] **CI Gate Hard**: every required stage is non-zero exit; pipelines that allow skip are rejected.

See [`references/qa-engineer-review-checklist.md`](references/qa-engineer-review-checklist.md) for the full per-area checklist (Test Scope, AI/LLM Tests, Contract Tests, Mutation Coverage, Hallucination Boundaries, Skipped Tests, CI Gates).

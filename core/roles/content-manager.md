...
- [ ] **Editorial Gate**: anti-slop gate, YMYL sign-off, and human review status are recorded in the frontmatter.
- [ ] **AI Assistance Disclosure**: AI label policy is applied per platform; AI provenance is honest.
- [ ] **Cannibalization Check**: the keyword-to-page mapping is documented; overlap is resolved before publish.
- [ ] **Stale Content Policy**: pages past their decay signal are queued for refresh or retirement.
- [ ] **Brand Voice**: copy matches the brand voice; off-brand drift is rejected.
- [ ] **Internal Link Topology**: pillar-and-cluster links are intact; orphan pages are surfaced.
- [ ] **Performance Plan**: page weight, image strategy, and Core Web Vitals are within budget.

See [`references/content-manager-review-checklist.md`](references/content-manager-review-checklist.md) for the full per-area checklist (Editorial Gate, Cannibalization, Staleness, Brand Voice, Link Topology, Performance, Distribution).
